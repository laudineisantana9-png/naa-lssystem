...
foreach($participants as $i=>$p){
    ...
    if(array_key_exists($uid,$transcriptStatus) && !$transcriptStatus[$uid]) $lines[]=['t'=>'   Transcrição automática: indisponível/não autorizada neste dispositivo.','kind'=>'muted'];
    $lines[]=['t'=>'','kind'=>'space'];
}
$lines[]=['t'=>'TRANSCRIÇÃO CRONOLÓGICA DA REUNIÃO','kind'=>'heading'];
$lines[]=['t'=>'A transcrição abaixo é produzida pelo reconhecimento de fala do navegador de cada participante, quando disponível e autorizado. Ela pode conter imprecisões e não substitui ata revisada.','kind'=>'muted'];
$lines[]=['t'=>'','kind'=>'space'];
if(!$transcripts){
